    ...
    public class UserAuthenticationAttribute : ActionFilterAttribute
    {
        ...
        public override void OnActionExecuting(ActionExecutingContext filterContext)
        {
            ...
            try
            {
                ...
                var company_id = Convert.ToInt32(Crypto.password_decrypt(filterContext.HttpContext.Request.Cookies["CompanyId"]));
                var access_level_id = Convert.ToInt32(Crypto.password_decrypt(filterContext.HttpContext.Request.Cookies["AccessLevelId"]));
                ...
                if (resp.Count > 0)
                {
                    foreach (var x in resp)
                    {
                        if (transaction_id != 0)
                        {
                            ...
                            if (x.enable_new)
                            {
                                top_menu += "<button class='btn btn-sm waves-effect waves-light btn-primary' onclick='window.location =\"/" + x.controller + "/" + x.action_new + "\"'><i class='icofont icofont-ui-edit'></i><strong>New Transaction</strong></button>"
                        ...
                        if (view)
                        {
                            view_menu = "<li class='breadcrumb-item'><a href=" + x.url + ">View</a></li>";
                        }
                ...
                controller.ViewBag.MenuView = view_menu;
